...
QLU 工具箱是一款面向齐鲁工业大学学生的本地校园效率桌面软件。v1.0.1 使用 Vue 3、TypeScript 与 Electron 提供全新桌面界面，Python 与 Playwright 负责可靠的本地自动化。当前内置“分项成绩导出”和“绩点计算器”两个教务工具，后续工具将通过统一模块规范逐步加入。
...
1. 前往 [Releases](https://github.com/C1ouDreamW/qlu-toolbox/releases) 下载最新版本：
   - 推荐下载 `QLUToolbox_v*_x64_Setup.exe`，运行安装程序完成安装。
   - 需要免安装使用时，下载 `QLUToolbox_v*.zip`，完整解压后运行 `QLUToolbox.exe`。
...
- 两种更新方式都会保留设置、任务记录、日志和浏览器登录状态；导出的文件位于用户选择的目录，也不会被更新程序删除。
...
- 登录等待时间为 15 分钟。
- 当前版本提供 Windows 64 位安装包和免安装 ZIP；安装包尚未进行数字签名，Windows 可能显示来源未知或 SmartScreen 提示。
