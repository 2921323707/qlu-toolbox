...
仓库同时维护 Manifest V3 浏览器扩展，源码位于 [`extension/`](extension/)。扩展版复用桌面端的 QLU 品牌、成绩字段与 GPA 规则，专注于用户已在浏览器中打开教务系统时的即时成绩分项查询。

进入扩展目录后，可运行以下命令完成校验和打包：
...
生成的浏览器扩展安装包位于根目录 `dist/toolbox-0.2.3.zip`，开发加载时请选择 `extension/` 目录。
